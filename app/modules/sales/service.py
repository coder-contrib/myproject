...
class SalesService:
    ...
    async def get_payments(self, invoice_id: UUID) -> list:
        return await self.payments.get_all(limit=200, invoice_id=invoice_id)
    ...
    async def approve_return(self, return_id: UUID) -> SalesReturn:
        sales_return = await self.returns.get_by_id_strict(return_id)
        if sales_return.status != "draft":
            raise AppException("Only draft returns can be approved", status_code=400)
